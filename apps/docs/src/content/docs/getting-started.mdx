...
<Callout type="info">
For multi-tenant setups where staff own their own resources, see [`resourceOwnerMode` and `staffProvisioning`](/configuration). The admin UI is fully translatable — see [Internationalization](/i18n).
</Callout>
