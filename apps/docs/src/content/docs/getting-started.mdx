...
description: Everything you need to install payload-reserve and have it running in your Payload CMS project.
---
...
When `userCollection` is set, the plugin injects `phone`, `notes`, and a `bookings` join field into your existing collection rather than creating a separate Customers collection. See [Collections → Customers](/collections#customers) for details.
