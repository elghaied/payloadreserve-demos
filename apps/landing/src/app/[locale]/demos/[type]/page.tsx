...
const demoConfigs: Record<DemoType, DemoPageConfig> = {
  salon: {
    ...
    features: [
      ...
      },
      {
        ...
        description: 'Define services (cut, colour, treatment) with exact durations; slot length adjusts automatically.',
      },
      {
        title: 'Buffer times',
      ...
      },
      {
        ...
        description: '24-hour minimum notice enforced server-side; late cancellations flagged for staff.',
      },
    ],
  ...
  defaultBufferTime: 5,            // 5 min between appointments
    ...
    features: [
      {
        ...
        description: 'Date-range booking with check-in and check-out. Conflict detection prevents double-booking.',
      },
      {
        title: 'Room types',
        description: 'Define room categories (Standard, Deluxe, Suite) each with their own availability and pricing.',
      },
      {
        title: 'Capacity management',
        description: 'Set per-room capacity and automatically prevent overbooking across your property.',
      },
      {
        title: 'Guest management',
        description: 'Capture guest details, special requests, and preferences in Payload collections.',
      },
    ],
    pluginSnippet: `payloadReserve({
  slugs: {
    ...
    features: [
      {
        ...
        description: 'Define service windows (lunch, dinner) and slot intervals. Guests pick from available times.',
      },
      {
        title: 'Party size',
        description: 'Guests specify their party size and the system matches them to an appropriately sized table.',
      },
      {
        title: 'Special requests',
        description: 'Guests add dietary requirements, occasion notes, or seating preferences at booking.',
      },
      {
        title: 'Auto table assignment',
        description: 'Tables are assigned automatically based on party size, availability, and capacity.',
      },
    ],
    pluginSnippet: `payloadReserve({
  slugs: {
    ...
    features: [
      {
        ...
        description: 'Each space has a maximum capacity. The system prevents overbooking automatically.',
      },
      {
        title: 'Multi-resource booking',
        description: 'Book multiple spaces or resources in a single reservation — great for complex events.',
      },
      {
        title: 'Event types',
        description: 'Configure different event categories (conference, wedding, concert) with distinct rules.',
      },
      {
        title: 'Ticketing integration',
        description: 'Hook into Stripe or your ticketing system to gate capacity with ticket sales.',
      },
    ],
    pluginSnippet: `payloadReserve({
  slugs: {
    ...
    <>
      ...
      <main className="pt-16 bg-[#FAFAF8] dark:bg-stone-900 min-h-screen">
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="flex items-center gap-5 mb-6">
              ...
              <div>
                ...
                <p className="text-base font-medium mt-1 text-violet-700 dark:text-violet-400">{config.tagline}</p>
              </div>
            </div>

            <p className="text-[#78716C] dark:text-stone-400 text-lg leading-relaxed max-w-2xl">{config.description}</p>
        ...
        <section className="py-20 px-6 lg:px-8 bg-white dark:bg-stone-800">
          <div className="max-w-4xl mx-auto">
            <p className="text-xs font-bold uppercase tracking-[0.2em] mb-3 text-violet-700 dark:text-violet-400">Features</p>
            ...
            <div className="grid sm:grid-cols-2 gap-6">
              {config.features.map((f) => (
                ...
                >
                  <h3 className="font-bold text-base mb-2 text-[#1C1917] dark:text-stone-100">{f.title}</h3>
                  <p className="text-sm text-[#78716C] dark:text-stone-400 leading-relaxed">{f.description}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="py-20 px-6 lg:px-8 bg-[#FAFAF8] dark:bg-stone-900">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="rounded-2xl border border-gray-200 dark:border-stone-700 bg-gradient-to-br from-gray-100 dark:from-stone-800 to-gray-50 dark:to-stone-700 aspect-video flex flex-col items-center justify-center gap-3 shadow-sm">
              ...
              <p className="text-sm font-medium text-gray-400 dark:text-stone-500">Screenshots coming soon</p>
              {isLive && (
                <p className="text-xs text-gray-400 dark:text-stone-500">
                  Visit the{' '}
                  <a href={config.liveUrl!} target="_blank" rel="noopener noreferrer" className="underline text-violet-700 dark:text-violet-400">
                    live demo
                  </a>{' '}
                  to see it in action
                </p>
        ...
        <section className="py-20 px-6 lg:px-8 bg-[#1C1917]">
          <div className="max-w-4xl mx-auto">
            <p className="text-xs font-bold text-amber-400 uppercase tracking-[0.2em] mb-3">Configuration</p>
        ...
        <section className="py-16 px-6 lg:px-8 bg-[#FAFAF8] dark:bg-stone-900 border-t border-gray-200 dark:border-stone-800">
          <div className="max-w-4xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-6">
            <div>
              <h3 className="font-bold text-[#1C1917] dark:text-stone-100 text-lg">Ready to explore {config.name}?</h3>
              <p className="text-[#78716C] dark:text-stone-400 text-sm mt-1">Try the live demo or request your own private environment.</p>
            </div>
