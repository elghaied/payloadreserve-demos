...
async function sendReservationEmail(
  ...
  try {
    ...
    if (!customer?.email) {
      payload.logger.warn(`No customer email for booking ${doc.id}`)
      return
    }
    ...
    const emailData = {
      customerName,
      eventName: (service?.name as string) || 'Event',
      venueName: (resource?.name as string) || 'Venue',
      date: dateStr,
      time: timeStr,
      ticketQuantity: (reservation as unknown as Record<string, unknown>).ticketQuantity as number | undefined,
      locale,
    }
    ...
    payload.logger.info(
      `Sent ${emailType} email for booking ${doc.id} to ${customer.email}`,
    )
  } catch (error) {
    payload.logger.error(
      `Failed to send ${emailType} email for booking ${doc.id}: ${error}`,
    )
  }
}
