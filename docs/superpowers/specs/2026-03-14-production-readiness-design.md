...
Ensure these patterns are present at the root level (be specific to avoid accidentally matching unrelated dotfiles):
```
.env
.env.local
.env.production
...
Note: The existing `.env*.local` pattern already covers `.env.local`, `.env.production.local`, etc. Adding the explicit entries above ensures all common variants are covered without the overly broad `.env.*` which could catch unrelated files.
...
**Scope:** `apps/salon/`
**Issues covered:** #2, #3, #4, #5, #6, #7, #8, #10, #11, #15
...
In `src/collections/Media.ts`, restrict allowed MIME types via Payload's built-in `mimeTypes` field:
```typescript
upload: {
  mimeTypes: ['image/jpeg', 'image/png', 'image/webp', 'image/gif', 'image/svg+xml'],
  disableLocalStorage: true, // uses S3
}
```

Note: Payload CMS 3.x does not have a `filesizeLimit` config field. File size limits must be enforced at the HTTP/middleware layer. Add a Next.js API route config to limit body size for upload routes:
```typescript
// In next.config.mjs, or via middleware that checks Content-Length header
// Reject uploads > 5MB by checking Content-Length before processing
```
Create a reusable middleware check in `src/lib/upload-size-limit.ts` that returns 413 if `Content-Length` exceeds 5MB. Apply to the Payload API catch-all route.
...
### 2i. GraphQL Playground (Issue #20) — NO ACTION NEEDED

Payload CMS 3.x already sets `graphQL.disablePlaygroundInProduction` to `true` by default (confirmed in Payload's type definitions). The auto-generated `src/app/(payload)/api/graphql-playground/route.ts` file must NOT be modified (it's regenerated by Payload). This issue is resolved by default Payload behavior — removing from the active fix list.

### 2j. Transaction Safety in Hooks

In `src/hooks/reservationNotifications.ts`, the `findByID` call at line 53 is missing the `req` parameter for transaction safety:
```typescript
const populatedReservation = await req.payload.findByID({
  collection: 'reservations',
  id: reservation.id,
  depth: 1,
  req, // Add this for transaction safety
})
```

This is the same fix applied to hotel in Phase 3.3 — salon has the identical bug.

### 2k. Sentry Integration (Issue #15)

1. Install `@payloadcms/plugin-sentry`, `@sentry/nextjs`
2. Create `sentry.client.config.ts` and `sentry.server.config.ts` with `Sentry.init({ dsn: process.env.NEXT_PUBLIC_SENTRY_DSN })`
3. Add to `payload.config.ts`:
   ...
   sentryPlugin({ Sentry })
   ```
   Note: The plugin requires a `Sentry` instance (not a DSN string). The DSN is configured in the Sentry init files, not in the plugin config.
4. Wrap `next.config.mjs` with `withSentryConfig()`
5. Add `NEXT_PUBLIC_SENTRY_DSN` and `SENTRY_AUTH_TOKEN` to `.env.example`
...
### 3.1 Apply all salon fixes (2a–2k)

Copy patterns from salon for: env validation, `.dockerignore`, Users access control, health check, security headers, rate limiting, upload limits, password generation, Sentry.
...
### 4.1 Apply all salon fixes (2a–2k)
...
### 5.1 Apply all salon fixes (2a–2k)
...
Note: Requests with **no Origin header** (e.g., from `curl`, server-side scripts) will pass through. This is intentional — the check blocks cross-site browser requests (CSRF) while allowing legitimate API usage from non-browser clients.
...
| 3 | Empty string env fallbacks | 2a, 3.1, 4.1, 5.1, 6.1 | Also covers #4 (same `requireEnv()` fix) |
| 4 | `NEXT_PUBLIC_SITE_URL` localhost fallback | 2a, 3.1, 4.1, 5.1 | Fixed as part of #3's `requireEnv()` |
...
| 10 | No file upload size limits | 2g, 3.1, 4.1, 5.1 | MIME types via Payload; size via middleware |
...
| 15 | No monitoring/observability | 2k, 3.1, 4.1, 5.1, 6.5, 7.3 | |
| 16 | No backup strategy | 8c | |
| 17 | Landing email failure silent | 6.8 | |
| 18 | Orphaned resource cleanup | 6.9 | |
| 19 | No CSRF on demo creation | 6.7 | Allows no-Origin requests (intentional) |
| 20 | GraphQL Playground exposed | N/A | **No action needed** — Payload defaults `disablePlaygroundInProduction: true` |
